...
from AgentMem.vector_stores.base import VectorStoreBase
...
class Qdrant(VectorStoreBase):
    ...
    def insert(self, vectors: list, payloads: list = None, ids: list = None):
        ...
        logger.info(f"collection_name:{self.collection_name},points:{points}")
...
def test():
    ...
    qdrant_store.insert(vectors)

    # Search for similar vectors
    query_vector = [[0.1 * i for i in range(128)]]
    results = qdrant_store.search(query="test", vectors=query_vector, limit=5)
    print("Search Results:", results)

    # Cleanup
    qdrant_store.delete_col()
