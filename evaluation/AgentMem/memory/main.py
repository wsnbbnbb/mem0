...
from AgentMem.configs.base import MemoryConfig, MemoryItem
from AgentMem.configs.enums import MemoryType
from AgentMem.configs.prompts import (
...
class Memory(MemoryBase):
    def __init__(self, config: MemoryConfig = MemoryConfig()):
        ...
        # Set default providers "ollama"
        self.config.embedder.provider =  "huggingface" 
        ...
        *,
        ...
        Args:
            ...
            infer (bool, optional): If True (default), an LLM is used to extract key facts from
                ...
                If False, 'messages' are added as raw memories directly.  模型决定是否推理处理记忆否则直接加
            ...
            prompt (str, optional): Prompt to use for the memory creation. Defaults to None. 判断是否是程序记忆，否则
        ...
        """
        logger.info(f"开始添加记忆，传入的参数, user_id: {user_id}, agent_id: {agent_id},\
                    run_id: {run_id}, memory_type: {memory_type}, infer: {infer},\
                    metadata: {metadata}, ")
        ...
        logger.info(f"初步处理数据结果:effective_filters {effective_filters}, Processed metadata: {processed_metadata}")
        ...
        if agent_id is not None and memory_type == MemoryType.PROCEDURAL.value:
            # add procedural memory,提示词在这里被传入
            results = self._create_procedural_memory(messages, metadata=processed_metadata, prompt=prompt)
            return results

        if self.config.llm.config.get("enable_vision"):
            logger.info(f"处理之前的视觉消息{messages}------")
            messages = parse_vision_messages(messages, self.llm, self.config.llm.config.get("vision_details"))
            logger.info(f"处理之后的视觉消息{messages}------")
        else:
            logger.info(f"不处理视觉消息,parse之前的{messages}------")
            messages = parse_vision_messages(messages)
            logger.info(f"不处理视觉消息,parse之后的{messages}------")
    ...
    def _add_to_vector_store(self, messages, metadata, filters, infer):
        logger.info(f"添加到向量库，消息内容为: {messages}, metadata: {metadata}, filters: {filters}, infer: {infer}")
        if not infer:  # 如果不需要生成记忆片段，则直接存储原始消息(按原样存储)
            returned_memories = []
        ...
        logger.info(f"Parsed messages 之后的消息:\n{parsed_messages}")
        # 2. 构建提示，使用最近消息窗口作为上下文
        if self.config.custom_fact_extraction_prompt:
            system_prompt = self.config.custom_fact_extraction_prompt
            user_prompt = f"Input:\n{parsed_messages}"
        else:
            # 获取默认**事实提取**提示词，并格式化消息
            system_prompt, user_prompt = get_fact_retrieval_messages(parsed_messages)
            
        logger.info(f"System Prompt [系统提示词] for Fact Extraction:{system_prompt}")
        logger.info(f"User Prompt [用户提示词] for Fact Extraction:{user_prompt}")
        # 3. 使用LLM提取事实（候选记忆）
        ...
        logger.info(f"根据提示词和消息生成回复: {response}")
        # 4. 解析LLM输出，获取候选记忆
        try:
        ...
        logger.info(f"New retrieved facts 新的检索事实[添加的信息]: {new_retrieved_facts}")
        ...
        for new_mem in new_retrieved_facts:
            ...
            logger.info(f"记忆库中存在该记忆{new_mem}相似的记忆: {existing_memories}")
            # 收集相似的现有记忆
            for mem in existing_memories:
                retrieved_old_memory.append({"id": mem.id, "text": mem.payload["data"]})
                
        logger.info(f"Retrieved old memories 检索到的旧记忆: \n{retrieved_old_memory},\n处理前数目: {len(retrieved_old_memory)}")
        logger.info(f"New message embeddings 新消息: {new_message_embeddings}")
        
        unique_data = {} # uuid - memory dict
        for item in retrieved_old_memory:
            unique_data[item["id"]] = item  # 使用字典去重，保留最新的相似记忆
        retrieved_old_memory = list(unique_data.values()) # 转回列表形式
        logger.info(f"Total existing memories[相似记忆数]: {len(retrieved_old_memory)}")
        
        logger.info(f" 列表数据：\n{retrieved_old_memory}，\n处理之后数目： {len(retrieved_old_memory)}")
        
        # mapping UUIDs with integers for handling UUID hallucinations
        # 映射UUID与整数以处理UUID幻觉
        temp_uuid_mapping = {} # 顺序索引uuid : idx- uuid
        for idx, item in enumerate(retrieved_old_memory):
            temp_uuid_mapping[str(idx)] = item["id"]
            retrieved_old_memory[idx]["id"] = str(idx) # 列表项变成idx - memory 
            
        logger.info(f"Temporary UUID mapping 临时UUID映射: {temp_uuid_mapping}")
        
        if new_retrieved_facts:
            function_calling_prompt = get_update_memory_messages(
                retrieved_old_memory, new_retrieved_facts, self.config.custom_update_memory_prompt
            )
            logger.info(f"函数工具调用提示词[]--需要对记忆采取的措施: {function_calling_prompt}")
            try:
                response: str = self.llm.generate_response(
                    messages=[{"role": "user", "content": function_calling_prompt}],
                    response_format={"type": "json_object"},
                )
                logger.info(f"函数工具提示词响应: {response}")
            except Exception as e:
            ...
            new_memories_with_actions = {}
        logger.info(f"New memories with actions 新的记忆和动作: {new_memories_with_actions}")
        returned_memories = []
        try:
            for resp in new_memories_with_actions.get("memory", []):
                
                logger.info(f'存储的记忆或动作？：{resp}')
                try:
                    ...
                    if event_type == "ADD":
                        ...
                        logger.info(f"resp.get(id):{resp.get('id')}\n,Updating memory : {temp_uuid_mapping}")
                        self._update_memory(
                    ...
                    import traceback
                    logger.error(f"Error processing memory action: {resp}, Error: {type(e).__name__}: {e}")
                    # 或者打印完整堆栈
        ...
        logger.info(f"Telemetry filters 过滤器 keys: {keys}, encoded_ids: {encoded_ids}")
        capture_event(
            "mem0.add",
            self,
            {"version": self.api_version, "keys": keys, "encoded_ids": encoded_ids, "sync_type": "sync"},
        )
        logger.info(f"最终返回的记忆结果: {returned_memories}")
        return returned_memories
    ...
    def _add_to_graph(self, messages, filters):
        ...
        if self.enable_graph:
            ...
            logger.info(f"添加到图数据库, data: {data}, filters: {filters}")
    ...
    def search(
        ...
        *,
        ...
        """
        logger.info(f"根据查询检索记忆: {query}, user_id: {user_id}, agent_id: {agent_id}\
                    , run_id: {run_id}, limit: {limit}, filters: {filters}, threshold: {threshold}")
        
        _, effective_filters = _build_filters_and_metadata(
            user_id=user_id, agent_id=agent_id, run_id=run_id, input_filters=filters
        )
        logger.info(f"Effective filters 有效过滤器: {effective_filters}")
        if not any(key in effective_filters for key in ("user_id", "agent_id", "run_id")):
            raise ValueError("At least one of 'user_id', 'agent_id', or 'run_id' must be specified.")
        ...
        logger.info(f"Search results 搜索结果: {original_memories}")
        logger.info(f"Graph entities 图实体: {graph_entities}")
        if self.enable_graph:
    ...
    def _search_vector_store(self, query, filters, limit, threshold: Optional[float] = None):
        logger.info(f"向量库检索: {query}, filters: {filters}, limit: {limit}, threshold: {threshold}")
        embeddings = self.embedding_model.embed(query, "search")
        memories = self.vector_store.search(query=query, vectors=embeddings, limit=limit, filters=filters)
        logger.info(f"检索到的记忆数: {len(memories)}, memories: {memories}")
        promoted_payload_keys = [
            "user_id",
            "agent_id",
            "run_id",
        ...
        for mem in memories:
            ...
            logger.info(f"Memory item dict 记忆项字典: {memory_item_dict}")
            for key in promoted_payload_keys:
                if key in mem.payload:
                    memory_item_dict[key] = mem.payload[key]
            ...
            if additional_metadata:
                memory_item_dict["metadata"] = additional_metadata
                logger.info(f"Additional metadata 额外的元数据: {additional_metadata}")
            if threshold is None or mem.score >= threshold:
                original_memories.append(memory_item_dict)
        logger.info(f"Filtered memories 过滤后的记忆: {original_memories}")
