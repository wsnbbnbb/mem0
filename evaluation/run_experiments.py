...
from src.memMini.add import MemoryADD_MINI
from src.memzero.search import MemorySearch
from src.memMini.search import MemorySearch_MINI
...
def main():
    ...
    parser.add_argument("--technique_type", choices=TECHNIQUES, default="mem0-mini", help="Memory technique to use")
    ...
    elif args.technique_type == "mem0-mini":
        if args.method == "add":
            memory_manager = MemoryADD_MINI(data_path="dataset/locomo10.json", is_graph=args.is_graph)
            memory_manager.process_all_conversations()
        elif args.method == "search":
            output_file_path = os.path.join(
                args.output_folder,
                f"mem0_mini_results_top_{args.top_k}_filter_{args.filter_memories}_graph_{args.is_graph}.json",
            )
            memory_searcher = MemorySearch_MINI(output_file_path, args.top_k, args.filter_memories, args.is_graph)
            memory_searcher.process_data_file("dataset/locomo10.json")
